...
public class AllItemsEditor : Editor
{
	...
	public static string[] AllItemsDescriptions
	{
		get
		{
	...
	private AllItems allItems;                        // Reference to the target.
	private string newItemDescription = "New Item";   // String to start off the naming of new Conditions.
	...
	private static void SetAllItemsDescriptions ()
	{
		// Create a new array that has the same number of elements as there are Conditions.
		AllItemsDescriptions = new string[TryGetItemsLength()];

		// Go through the array and assign the description of the condition at the same index.
		for (int i = 0; i < AllItemsDescriptions.Length; i++)
		{
			AllItemsDescriptions[i] = TryGetItemAt(i).description;
		}
	}
	...
	public static Item TryGetItemAt (int index)
	{
		// Cache the AllConditions array.
		Item[] allItems = AllItems.Instance.items;

		// If it doesn't exist or there are null elements, return null.
		if (allItems == null || allItems[0] == null)
			return null;
	...
	public static int TryGetItemsLength ()
	{
		// If there is no Conditions array, return a length of 0.
		if (AllItems.Instance.items == null)
			return 0;

		// Otherwise return the length of the array.
		return AllItems.Instance.items.Length;
	}
}
